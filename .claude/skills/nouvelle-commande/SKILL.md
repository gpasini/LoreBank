...
4. L'action, dans le controller du module (dérivé de `ModuleController`) :
   `SendAsync(command)` → 204, ou `CreateAsync(command, actionName:
   nameof(GetById))` → 201 + `Location` et corps vide. Sur une route mixte, la
   route est autoritaire : `command with { AccountId = id }` écrase un champ
   posté en double.
