...
> = {
  ...
  "BANK.NON_EMPTY_ACCOUNT_CLOSURE": () =>
    "Un compte ne se ferme qu'à solde nul.",
  CONCURRENT_UPDATE: () =>
    "Cet élément a été modifié entre-temps. Rechargez-le et recommencez.",
  CURRENCY_MISMATCH: ({ left, right }) =>
    `Opération impossible entre ${String(left)} et ${String(right)}.`,
  INVALID_ACTOR: ({ actor }) =>
    `« ${String(actor)} » n'est pas un identifiant d'acteur valide.`,
  INVALID_BIC: ({ bic }) => `« ${String(bic)} » n'est pas un BIC valide.`,
  INVALID_CURRENCY: ({ currency }) =>
    `« ${String(currency)} » n'est pas une devise connue.`,
  INVALID_IBAN: ({ iban }) => `« ${String(iban)} » n'est pas un IBAN valide.`,
  INVALID_PAGING: ({ maxPageSize }) =>
    `La page demandée n'existe pas (au plus ${String(maxPageSize)} éléments par page).`,
  INVALID_SIGNAL_RESOURCE: ({ resource }) =>
    `« ${String(resource)} » n'est pas une ressource de Signal valide.`,
  "LEDGER.EMPTY_JOURNAL_ENTRY": () =>
    "Une écriture comptable ne peut pas être vide.",
  "LEDGER.INVALID_LEDGER_ACCOUNT_REF": ({ value }) =>
    `« ${String(value)} » n'est pas une référence de compte.`,
  "LEDGER.JOURNAL_ENTRY_NOT_FOUND": () =>
    "Cette écriture comptable est introuvable.",
  "LEDGER.UNBALANCED_JOURNAL_ENTRY": () =>
    "L'écriture comptable n'est pas équilibrée.",
  "LEDGER.UNKNOWN_BANK_ACCOUNT": () =>
    "Aucune écriture comptable pour ce compte.",
